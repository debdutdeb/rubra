...
class Milvus(VectorStore):
        ...
        if metadatas is not None:
            for d in metadatas:
                for key in self.fields:
                    if key not in [self._text_field, self._vector_field]:
                        value = d.get(
                            key, ""
                        )  # Use a default value (e.g., None) if the key is not present in the metadata
    ...
    def delete_entities(self, expr: str):
        if self.col is None:
